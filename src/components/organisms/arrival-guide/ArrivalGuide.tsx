          ...
          <div className="flex min-h-full items-center justify-center p-4">
            <DialogPanel className="w-full max-w-md rounded-2xl bg-white shadow-2xl overflow-hidden">
              ...
              <div className="flex flex-col gap-3 p-4">
                <div className="">
                  {step === 1 && <Step1Transport address={address} />}
                  {step === 2 && <Step2Parking parkings={data.parkings} />}
                  {step === 3 && (
                    <Step3Access
                      accessInstructions={data.access_instructions}
                      isTranslated={data.isAccessInstructionsTranslated}
                    />
                  )}
                  {step === 4 && (
                    <Step4Essentials
                      wifi={data.wifi}
                      isWifiTranslated={data.isWifiTranslated}
                    ...
                    {visibleSteps.map((s) => (
                      <div
                        ...
                        className={`h-1 rounded-full transition-all duration-300 ${
                          s === step ? "w-6 bg-primary-500" : "w-2 bg-gray-200"
                        }`}
                      />
                    ))}
                  </div>
              ...
              <div className="flex justify-between gap-2 p-4 border-t border-t-gray-100 bg-primary-50">
                ...
                {!isLast ? (
                  <Button
                    ...
                    label={t("next")}
                    color="primary"
                    iconRight={<IconArrowRightAlt size={20} />}
                  />
                ) : (
                  <Button
                    onClick={handleClose}
