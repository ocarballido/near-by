...
const LanguageSelector = ({
    ...
    return (
        <div ref={dropdownRef} className="relative">
            ...
            <DropdownMenu
                open={open}
                className="absolute z-50 right-0 top-11 gap-0.5!"
            >
                {LOCALES.map((language: string) => {
                    ...
                    return (
                        <Link
                            ...
                            key={language}
                            locale={language}
                            className={`rounded-sm w-full transition-all flex items-center gap-0 hover:bg-primary-50 hover:text-primary-500 hover:cursor-pointer font-medium text-sm py-2 px-3 shrink-0 ${languageStyles}`}
                        >
