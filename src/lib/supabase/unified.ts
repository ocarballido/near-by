...
export class SassClient {
    ...
    async signInWithMagicLink(email: string, locale: string = "en") {
        ...
        // El locale va en el primer nivel de la URL de redirect, porque es ahí
        // donde la Edge Function (send-email) lo lee desde email_data.redirect_to.
        // El destino interno de la app (innerRedirect) viaja aparte, anidado.
        const innerRedirect = encodeURIComponent(`/app?fromAuth=1`);
        const redirectUrl = `${origin}/auth/callback?locale=${encodeURIComponent(
            locale,
        ...
        const { data, error } = await this.client.auth.signInWithOtp({
            ...
            options: {
                ...
                // Si el usuario es nuevo (passwordless signup), esto también
                // persiste el locale en user_metadata para futuros envíos.
                data: { language: locale },
            },
        });
