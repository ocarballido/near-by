...
export async function translateAndStoreDetails(
    ...
    const validFields = fields.filter((f) => {
        ...
        if (f.fieldKey === "name") return f.value.trim().length > 2;
        return f.value.trim().length > 10;
    });
    if (validFields.length === 0) return;

    console.log(
        "[translateAndStoreDetails] propertyDetailId:",
        propertyDetailId,
    );
    console.log(
        "[translateAndStoreDetails] validFields:",
    ...
    for (const field of validFields) {
        ...
        const { error } = await supabase
            ...
            .upsert(upsertRows, {
                onConflict: "property_detail_id,lang,field_key",
            });

        console.log(
            "[translateAndStoreDetails] upsert result:",
            JSON.stringify(error),
        );

        if (error) {
            console.error("[translateAndStoreDetails] Error en upsert:", error);
        }
    }
}
...
${cleanText}`;
    ...
    try {
        ...
        if (!response.ok) {
            console.error(
                "[translateAndStoreDetails] Claude API error:",
                response.status,
            );
            return null;
        }
        ...
        console.error(
            "[translateAndStoreDetails] Error parseando respuesta:",
            err,
        );
        return null;
    }
