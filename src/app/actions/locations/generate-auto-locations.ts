...
export async function generateAutoLocations(
    propertyId: string,
    selections: AutoLocationSelection[],
): Promise<GenerateAutoLocationsState> {
    try {
        const parsed = Schema.safeParse({ propertyId, selections });
        if (!parsed.success) {
        ...
        // ✅ filtramos selecciones que realmente tienen categoryId conocido,
        // arrastrando el count elegido por el usuario junto al par subCategoryId/categoryId.
        const supported = selections
            .map(({ subCategoryId, count }) => {
                const categoryId = findCategoryIdBySubCategoryId(subCategoryId);
                return categoryId ? { subCategoryId, categoryId, count } : null;
            })
            .filter(notNull);
