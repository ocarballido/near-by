...
export async function fetchSubcategoryPageData(
  ...
  for (const t of translations ?? []) {
    ...
    translationMap.get(t.property_data_id)![t.field_key] = t.translated_value;
  }
...
export async function fetchInfoSectionsData(
  ...
  const validItems = mergedItems.filter(
    (i): i is { id: string; description: string; sub_category_id: string } =>
  ...
  return validItems
    .map((i) => ({
      ...
      isTranslated:
        translationMap.has(i.id) && !!translationMap.get(i.id)?.description,
    }))
...
export async function fetchArrivalGuideData(
  ...
  return {
    ...
    access_instructions:
      ...
      null,
    isAccessInstructionsTranslated: translationMap.has("access_instructions"),
