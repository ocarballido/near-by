...
import { LOCALES, type Locale } from "@/config/config-constants";
...
const emptyContentByLocale = (): Record<Locale, LocaleContent> =>
    Object.fromEntries(LOCALES.map((loc) => [loc, emptyContent()])) as Record<
        Locale,
        LocaleContent
    >;
...
// Record exhaustivo a propósito: si añadís un locale a config-constants.ts
// sin actualizar esta lista, TypeScript falla en build, no en producción.
...
export default function BroadcastForm() {
    ...
    const handleSubmit = async () => {
        ...
        // Cada locale usa su propio contenido si tiene subject; si no,
        // cae a ES. Generalizado sobre LOCALES en vez de tres constantes
        // sueltas, para que añadir un locale nuevo no requiera tocar esto.
        const contentByLocale = Object.fromEntries(
            LOCALES.map((loc) => [
                loc,
                loc === "es" || !content[loc].subject
                    ? esContent
                    : parseContent(content[loc]),
            ]),
        ) as Record<Locale, ReturnType<typeof parseContent>>;
        ...
        try {
            ...
            if (!res.ok) {
                ...
                setResult(data);
                setContent(emptyContentByLocale());
                setImageUrl("");
                setCtaUrl("");
    ...
    return (
        <div className="max-w-2xl mx-auto p-8">
            ...
            <p className="text-gray-500 text-sm mb-8">
                ES es obligatorio. El resto de idiomas usan ES como fallback si
                se dejan vacíos.
            </p>
